id integer NOT NULL DEFAULT nextval('despesas_adm_pessoal_id_seq'::regclass),
    ...
    CONSTRAINT despesas_adm_pessoal_pkey PRIMARY KEY (id),
    CONSTRAINT despesas_adm_pessoal_fluxo_caixa_id_fkey FOREIGN KEY (fluxo_caixa_id)
...
ALTER TABLE IF EXISTS public.despesas_adm_pessoal
    OWNER to postgres;

-- trigger_atualizar_fluxo_caixa

-- DROP TRIGGER IF EXISTS trigger_atualizar_fluxo_caixa ON public.despesas_adm_pessoal;

CREATE OR REPLACE TRIGGER trigger_atualizar_fluxo_caixa
    AFTER INSERT OR UPDATE OF valor_pago
    ON public.despesas_adm_pessoal
    FOR EACH ROW
